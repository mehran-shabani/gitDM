...
def make_superuser(**extra: Any) -> tuple[object, str]:
    """
...
def create_ai_summary(patient: object | None = None, model_label: str = "note", created_at: datetime | None = None) -> AISummary:
    """
...
def build_changelist_request(path: str, user: object, params: dict[str, Any] | None = None) -> HttpRequest:
    """
    ...
    پارامترها:
        ...
        params (dict | None): دیکشنری پارامترهای کوئری؛ مقادیر لیستی نیز پشتیبانی می‌شوند و با urlencode (doseq=True) به رشتهٔ کوئری تبدیل می‌شوند.
...
def get_admin_changelist_url(model: object) -> str:
    """
...
def test_resource_type_filter_lookups_only_unique_non_empty_models(admin_client: Client) -> None:
...
def test_resource_type_filter_queryset_filters_when_value_selected(admin_client: Client) -> None:
...
def test_resource_type_filter_queryset_returns_unfiltered_when_no_value(admin_client: Client) -> None:
    create_ai_summary(model_label="alpha")
    create_ai_summary(model_label="beta")
    request = admin_client.request().wsgi_request
    ma = AISummaryAdmin(AISummary, admin.site)
...
def test_changelist_view_renders_with_result_list_marker(client: Client) -> None:
...
def test_changelist_view_handles_non_renderable_response_gracefully(monkeypatch: Any) -> None:
    ...
    def fake_changelist_view(self: Any, request: Any, extra_context: Any = None) -> Any:
        """
        واژهٔ کوتاه:
    ...
    def super_stub(self: Any, request: Any, extra_context: Any = None) -> Any:
        """
...
def test_created_at_filter_and_date_range_behaviour(client: Client) -> None:
...
def test_resource_type_filter_reads_value_from_params_and_filters(monkeypatch: Any) -> None:
    """
    Ensure SimpleListFilter reads selected value from request.GET/params and filters queryset accordingly.
    """
    # Seed two types
    create_ai_summary(model_label="alpha")
    create_ai_summary(model_label="beta")
    ...
    rf = RequestFactory()
    req = rf.get("/", {"resource_type": "alpha"})
    ma = AISummaryAdmin(AISummary, admin.site)

    # Pass request and its params to the filter
    flt = AISummaryResourceTypeFilter(req, req.GET.copy(), AISummary, ma)

    assert flt.value() == "alpha"
    qs = flt.queryset(req, AISummary.objects.all())
    assert qs.count() == 1
    assert qs.first().content_type.model == "alpha"


def test_resource_type_filter_lookups_empty_when_no_records(admin_client: Client) -> None:
    """
    When there are no AISummary rows, the filter should expose no lookups.
    """
    AISummary.objects.all().delete()
    request = admin_client.request().wsgi_request
    ma = AISummaryAdmin(AISummary, admin.site)
    flt = AISummaryResourceTypeFilter(request, {}, AISummary, ma)
    lookups = flt.lookups(request, ma)
    assert isinstance(lookups, list | tuple)
